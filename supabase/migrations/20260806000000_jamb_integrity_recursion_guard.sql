-- Migration: JAMB ALOC Sync Integrity & Recursion Guard

-- 1. Structural constraints & columns on past_questions
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'chk_pq_year_range'
  ) THEN
    ALTER TABLE public.past_questions
      ADD CONSTRAINT chk_pq_year_range CHECK (year BETWEEN 2009 AND 2024);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'chk_pq_answer_letter'
  ) THEN
    ALTER TABLE public.past_questions
      ADD CONSTRAINT chk_pq_answer_letter
      CHECK (correct_answer IS NULL OR correct_answer IN ('a','b','c','d','e'));
  END IF;
END $$;

CREATE UNIQUE INDEX IF NOT EXISTS uq_pq_content_hash ON public.past_questions (content_hash);

-- 2. jamb_sync_jobs columns & unique index
...
GRANT ALL ON public.jamb_integrity_audit_log TO authenticated;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Admins can view and insert audit logs'
  ) THEN
...
BEGIN
  ...
  -- 1. True duplicates: keep the oldest row per content_hash, delete the rest.
  WITH ranked AS (
  ...
  -- 2. Missing options/answer: quarantine + deactivate visibility
  ...
  -- 3. Out-of-range years: quarantine, don't delete
  UPDATE past_questions
  ...
  -- 4. Stuck / looping sync jobs: pause them
  ...
  -- 5a. Orphan visibility rows pointing at questions that no longer exist
  ...
  -- 5b. Backfill missing visibility rows for active admins on clean, active questions
...
-- 6. Clean view for student queries
