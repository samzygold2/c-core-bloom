-- Migration: Database Storage Level & Capacity Stats RPC
...
BEGIN
  ...
  SELECT jsonb_agg(row_to_json(t)) INTO tables_info FROM (
    SELECT 
      ...
      (SELECT count(*) FROM past_questions WHERE table_name = 'past_questions') as pq_exact_count
  ...
  SELECT count(*) INTO test_submissions_count FROM public.test_submissions;
